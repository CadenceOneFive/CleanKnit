...
select
    'CREATE VIRTUAL TABLE "vsv_' || substring(substring(d.name, length(d.name) -12), 1, 9) || '" IF NOT EXISTS USING vsv(filename="' || d.name || '",header=yes,nulls=on,affinity=numeric);'
FROM
...
where
    ...
    -- filenames that are not in the 4-4.csv form (although note that the regexp package
    ...
    regexp_like(d.name, '[a-z0-9]+-[a-z0-9]+.csv$');